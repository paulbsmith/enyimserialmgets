...
namespace Enyim.Caching
{
	...
	public partial class MemcachedClient : IMemcachedClient, IMemcachedResultsClient
	{
        ...
        public bool MGetInSerial { get; set; }
		...
		protected virtual IDictionary<string, T> PerformMultiGet<T>(IEnumerable<string> keys, Func<IMultiGetOperation, KeyValuePair<string, CacheItem>, T> collector)
		{
			if (MGetInSerial)
			{
				return PerformMultiGetSerial(keys, collector);
			}
			else
			{
				return PerformMultiGetParallel(keys, collector);
			}
		}

		protected virtual IDictionary<string, T> PerformMultiGetSerial<T>(IEnumerable<string> keys, Func<IMultiGetOperation, KeyValuePair<string, CacheItem>, T> collector)
		{
		...
		protected virtual IDictionary<string, T> PerformMultiGetParallel<T>(IEnumerable<string> keys, Func<IMultiGetOperation, KeyValuePair<string, CacheItem>, T> collector)
		{
